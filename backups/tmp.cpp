...
// ============================================================
const char* ssid     = "Digi-Ashu";
const char* password = "DigiAshu123";
// ============================================================
...
String lastLcdDirection = "";

const int   deadZone        = 1800;
const int   neutralDeadZone = 3000;
const int   axisThreshold   = 3500;
...
// ============================================================
//  LCD — refresh doar la schimbarea directiei
// ============================================================
void updateLcdDirection(const String &direction) {
  if (direction == lastLcdDirection) return;
  lastLcdDirection = direction;

  lcd.clear();
  lcd.setCursor(0, 0);
  if      (direction == "NEUTRU")        lcd.print("  -- NEUTRU --  ");
  else if (direction == "FATA")          lcd.print("    ^ FATA ^    ");
  else if (direction == "SPATE")         lcd.print("    v SPATE v   ");
  else if (direction == "STANGA")        lcd.print("  < STANGA      ");
  else if (direction == "DREAPTA")       lcd.print("     DREAPTA >  ");
  else if (direction == "FATA-STANGA")   lcd.print(" ^ FATA-STANGA  ");
  else if (direction == "FATA-DREAPTA")  lcd.print(" FATA-DREAPTA ^ ");
  else if (direction == "SPATE-STANGA")  lcd.print(" v SPATE-STANGA ");
  else if (direction == "SPATE-DREAPTA") lcd.print("SPATE-DREAPTA v ");
...
String getDirectionLabel(long dx, long dy) {
  long absDx = abs(dx);
  long absDy = abs(dy);
  if (absDx < neutralDeadZone && absDy < neutralDeadZone) return "NEUTRU";
  if (absDx > absDy * (1.0f + diagonalRatio) && absDx > axisThreshold)
    return dx > 0 ? "STANGA" : "DREAPTA";
  if (absDy > absDx * (1.0f + diagonalRatio) && absDy > axisThreshold)
